...
Where a third party processes data for its own purposes — such as Lemon Squeezy's checkout and billing as merchant of record, or a cloud provider you enable under your own account and API key — that processing is carried out under the responsibility of that third party, not DV Studio (see Section 3). For the processing this policy attributes to DV Studio — the license-validation calls, opt-in crash reporting, the update check and its one-time Microsoft Store install report, the speech-model download, support correspondence, and the websites — DV Studio is the controller (see Sections 3, 5, 6 and 7).
...
- **A one-time Microsoft Store install report** (one request to updates.voicewink.app carrying your IP address, a User-Agent naming VoiceWink, and the update channel as part of the address — which states that a matching Microsoft Store installation event was found for this copy) — to Cloudflare, only when VoiceWink finds a matching Microsoft Store installation event, after an automatic update check, and never while automatic update checking is off. See §4.5.
...
Local data does not leave your device unless you enable a third-party cloud feature at your own risk and under your own responsibility — except for four automatic network calls: license validation to Lemon Squeezy (Section 3 Category A, required to keep your activation valid), the update check to Cloudflare (Section 4, which you can turn off at any time in the app), the one-time Microsoft Store install report to Cloudflare (Section 4.5, sent only after an automatic update check, so it is off whenever that check is off), and the speech-model download from models.voicewink.app, also served by Cloudflare — or, only if that endpoint cannot deliver the file correctly, from Hugging Face (next paragraph; only when a selected model is not yet on your device).
...
- **Cloudflare** — operator of the updates.voicewink.app update-check endpoint described in Section 4, of the models.voicewink.app speech-model download endpoint described in Section 2, and of the website hosting and measurement described in Section 5. Cloudflare hosts the static update manifest and the speech-model files on its R2 object storage and serves them via its global edge network on DV Studio's behalf, and provides the aggregated request statistics from which DV Studio reads the daily totals described in Section 4.2. Where Cloudflare processes personal data on behalf of DV Studio, DV Studio has entered into a data-processing agreement with Cloudflare under which this processor provides adequate guarantees that the processing of your personal data complies with the requirements of the GDPR, national regulations and this Privacy Policy in order to protect your rights.
...
No audio, text, or transcribed content leaves your device — with four automatic exceptions: the license-validation egress to third-party Lemon Squeezy in Section 3 Category A (cannot be disabled while a paid license is active; required for the license to remain valid), the update-check egress in Section 4 (which you can turn off at any time in the app), the one-time Microsoft Store install report in Section 4.5 (off whenever the automatic update check is off), and the speech-model download described in Section 2 (which happens only when a local model you selected is missing). None of the four ever carries audio, text, or transcribed content.
...
No transcribed text, audio, API keys, license keys, or local identifiers other than the four items above are sent. (The separate one-time install report is described in 4.5.)

### 4.2 Who receives it

The endpoint is served by **Cloudflare** from a Cloudflare R2 bucket controlled by DV Studio. Cloudflare's default request logs (which may include source IP, timestamp, request path, and basic HTTP metadata) are governed by Cloudflare's privacy policy and retained per Cloudflare's defaults. The manifest is a static object served by R2, and DV Studio does **not** log individual update requests. From Cloudflare's aggregated request statistics for this endpoint, DV Studio counts per day how many update checks, installer downloads and install reports (4.5) it received, and keeps only those daily totals — no IP address or other request detail.

### 4.3 Legal basis

Update checking relies on **Article 6(1)(b) and (f) GDPR** — performance of the contract, because the updates it finds are included with your activation key (see the EULA, section 12.2), and **legitimate interests**, balancing your interest in receiving security and bug-fix updates against the minimal data collected. The data minimization is structural: no app-level analytics other than the one-time install report in 4.5, no separately-retained logs, and the user-facing automatic-update-check setting is honored before the network call is made.
...
### 4.5 One-time Microsoft Store install report

When VoiceWink finds that a copy was installed from the Microsoft Store, it tells DV Studio once, so that DV Studio can count installations made through that channel. This applies to the public release channel only.

**What is read, on your device.** At the first automatic update check for which VoiceWink holds no saved result for this installation — normally the first after it is installed, or after it is updated to a version that includes this report — it reads the Microsoft Store installation events that Windows recorded in its Microsoft Store event log during a limited period around the time the VoiceWink installation folder was created. Those events can concern other products; they are inspected on your device only to look for the event recording that VoiceWink's Microsoft Store product finished installing, and they are neither kept nor sent. The result, together with the installation folder's creation time it belongs to and whether the report has been delivered, is kept in your local settings. It can be wrong (for example, when VoiceWink was installed from the Microsoft Store and reinstalled another way shortly afterwards) or unavailable (for example, when Windows no longer holds the event).

**What is sent.** If a matching event is found, VoiceWink sends an HTTPS GET to `https://updates.voicewink.app/<channel>/install-source/msstore` after an automatic update check has succeeded. The request carries your IP address (visible to Cloudflare as the operator of the endpoint, as for any HTTPS request), a User-Agent naming VoiceWink, the release channel (part of the address, e.g. win-x64-stable), and the fact the rest of the address states — that a matching Microsoft Store installation event was found — and nothing else: no version, no license key, no installation or device identifier. VoiceWink stops once it has received any HTTP response and saved that result, and makes at most three attempts for each installation record it keeps. An interrupted delivery or a failure to save its result can produce a duplicate report, and deleting VoiceWink's local settings can allow a second report. When no matching event is found, nothing is sent.

**How it is used.** DV Studio counts the reports Cloudflare receives each day — a count of reports received, not of installations completed that day — from Cloudflare's aggregated request statistics, and keeps only those daily totals (Section 4.2).

**Legal basis and your control.** This relies on DV Studio's legitimate interests (Article 6(1)(f) GDPR) in knowing how many copies of the Software are installed through the Microsoft Store, balanced against the limited data involved: one fact, normally reported once per installation, with no installation or device identifier. It follows the automatic update-check setting (4.4): while automatic update checking is off, the event log is not read and nothing is sent. A manual update check never sends it.
...
- **Web analytics (cookieless).** Two cookieless measurement tools run on Cloudflare's network, which already serves the sites; the visit data goes to no other processor. **Cloudflare Web Analytics** reports only aggregate, non-identifying statistics derived from short-lived performance-timing readings; it does not fingerprint visitors and your IP address is discarded at Cloudflare's edge. **DV Studio's own page beacon**, a script served from s.voicewink.app and running on voicewink.app only, records which of its pages are opened, the referring page or the source tag carried by a link DV Studio published (one of a fixed list of values naming where the link was posted, such as its Microsoft Store listing or a button inside the Software — not who the visitor is), approximate country, how long each page was visible, and clicks on the Download and Buy links. It stores no IP address, sets no cookie, and writes nothing to your device. To recognize two page views as one visit it derives a short-lived identifier by hashing the IP address with the browser's user-agent string, the current date (UTC), and a secret value held only on our server; the identifier is the same for all of a visitor's page views that day, changes at midnight (UTC), and nothing DV Studio keeps links it across days. This allows DV Studio to see an individual visit as a sequence of pages and the time spent on each, but not to identify the visitor. DV Studio cannot tell which beacon rows are yours from the data it keeps, so it cannot apply an access or erasure request to them unless you provide information that identifies them (Article 11 GDPR); how to object to future collection is described in the next bullet, "Opting out of the page beacon". Legal basis: **legitimate interests** (Article 6(1)(f) GDPR) in understanding how the sites are used. The websites do not use advertising or cross-site trackers, and analytics data is not linked to any other data about you. Visit-level beacon data is retained for three months on Cloudflare's systems; daily totals without visit-level detail are retained for as long as the sites operate.
- **Opting out of the page beacon.** You can stop the page beacon from counting you at any time with the opt-out switch on the website's Cookie Notice page (voicewink.app/cookies), or by setting the value `vw-notrack` to `1` in your browser's local storage for voicewink.app. The choice lasts until that site's data is cleared from your browser, whether by you or by the browser itself (a private window clears it when closed); a browser that blocks local storage for voicewink.app is not counted by the beacon at all. You can also object by writing to the contact in Section 1 (Article 21 GDPR).
...
- **Legitimate interest** (Article 6(1)(f)) for security logging, fraud-prevention measures attached to license activation, the automatic update check described in Section 4, the one-time Microsoft Store install report described in Section 4.5, the download of speech-model files you have selected (Section 2), and the website hosting and analytics described in Section 5.
...
Cloudflare's logs for the update and speech-model endpoints follow Cloudflare's standard retention; DV Studio keeps no update-check, install-report or model-download records of its own, only the daily totals described in Section 4.2. A fallback model-download request to Hugging Face is governed by Hugging Face's own privacy policy (Section 2). Website measurement data is retained as described in Section 5.
...
- **Network calls** (license validation, optional cloud features, the update check, the one-time install report, the speech-model download) use encrypted (TLS) connections.
