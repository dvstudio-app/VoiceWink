...
Cloudflare's logs for the update and speech-model endpoints follow Cloudflare's standard retention; DV Studio does not separately retain update-check or model-download records. A fallback model-download request to Hugging Face is governed by Hugging Face's own privacy policy (Section 2). Website measurement data is retained as described in Section 5.
